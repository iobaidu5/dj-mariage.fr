    ...
    $.ajax({
      ...
      success: function (data, response, message) {
        if (type === "contact") {
          ...
          e.target.querySelector("#contact-form-response").innerHTML = successMessage;
        }
        if (type === "devis") {
          e.target.querySelector("#devis-form-response").classList.add("success");
          e.target.querySelector("#devis-form-response").innerHTML = successMessage;
        }
        window.setTimeout(() => {
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        if (type === "contact") {
          ...
          e.target.querySelector("#contact-form-response").innerHTML = ErrorMessage;
        }
        if (type === "devis") {
          e.target.querySelector("#devis-form-response").classList.add("error");
          e.target.querySelector("#devis-form-response").innerHTML = ErrorMessage;
...
var CaptchaCallback = function () {
  jQuery(".g-recaptcha").each(function () {
    grecaptcha.render(this, {
      sitekey: "6LddnwMbAAAAAP4OqDnwCPTaFGSiP14Hv53_lgo9",
